...
public interface ActivitySignupMapper {
    ...
    // ⭐ 新增 VO 查询（包含活动标题 + 用户姓名）
    List<ActivitySignupVO> selectSignupVOsByActivityId(@Param("activityId") Integer activityId);

    // ⭐（可选补充）查询未审核的 VO
    List<ActivitySignupVO> selectPendingSignupVOs(@Param("activityId") Integer activityId);

    // ⭐（可选补充）查询已通过的 VO
    List<ActivitySignupVO> selectApprovedSignupVOs(@Param("activityId") Integer activityId);
    /**
     * 根据活动ID和用户ID查询报名记录数量
     * @param activityId 活动ID
     * @param userId 用户ID
